...
<div id="container">
    ...
    <form id='TrainerCertificate' method='post' name='TrainerCertificate' action='save' charset='utf-8'>
        ...
        <table>
            ...
            <tr><td><label for='certificate'><fmt:message key='certificate'/></label></td>
                <td>
                    <select required='required' class='inpReq' name='certificate' id='certificate'>
                        <option value=''><fmt:message key='not_selected'/></option>
                        <c:forEach items='${certificates}' var='item'>
                            <option value='${item.id}' <c:if test='${item.id==bean.certificate.id}'>selected='selected'</c:if> >${item.name}</option>
                        </c:forEach>
                    </select>
                    <div class='errorMessage'>${messageMap.certificate}</div>
                </td></tr>
        ...
        <input type='hidden' name='required' value='<%="certificate:reqTrue:typeDic^"
